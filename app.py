...
def fetch_offres(token, keywords, departement, type_contrat):
    """
    L'API France Travail est limitée à 150 résultats par appel (range 0-149).
    Stratégie pour récupérer plus : on multiplie les appels avec des variantes
    de mots-clés et on déduplique par ID d'offre.
    """
    url = "https://api.francetravail.io/partenaire/offresdemploi/v2/offres/search"
    headers = {"Authorization": f"Bearer {token}", "Accept": "application/json"}

    # Variantes de mots-clés pour contourner la limite 150
    keyword_variants = [keywords]
    base = keywords.lower().strip()
    if "data" in base:
        keyword_variants += [
            keywords + " engineer",
            keywords + " analyst",
            ...
            keywords + " pipeline",
            "ingénieur " + keywords,
            "chef de projet " + keywords,
        ]

    all_offres = {}  # dict id -> offre pour déduplication automatique
    progress = st.progress(0, text="Récupération des offres en cours...")

    for idx, kw in enumerate(keyword_variants):
        progress.progress(int((idx / len(keyword_variants)) * 100),
                          text=f"Recherche variante {idx+1}/{len(keyword_variants)} : « {kw} »")
        range_start = 0
        while True:
            range_end = range_start + 49
            if range_end > 149:
                break  # limite hard de l'API

            params = {"motsCles": kw, "range": f"{range_start}-{range_end}"}
            if departement:
            ...
            if r.status_code in (200, 206):
                ...
                for o in offres:
                    all_offres[o.get("id", "")] = o  # déduplication par ID
                if len(offres) < 50:
                    break
                range_start += 50
    ...
    progress.progress(100, text=f"✅ {len(all_offres)} offres uniques récupérées !")
    import time; time.sleep(0.5)
    progress.empty()

    return list(all_offres.values())
...
if search_btn:
    if not client_id or not client_secret:
        ...
        with st.spinner(f"Recherche des offres « {keywords} »..."):
            offres_raw = fetch_offres(token, keywords, departement, type_contrat)
        ...
        if not offres_raw:
            ...
            st.divider()
            st.subheader("📄 Descriptions des postes")
            st.caption("Clique sur une offre pour lire la description complète")

            for _, row in df_filtered.iterrows():
                label = f"**{row['Titre']}** — {row['Entreprise']} | {row['Localisation']} | {row['Contrat']} | {row['Date']}"
                with st.expander(label):
                    col_a, col_b = st.columns([2, 1])
                    with col_a:
                        st.markdown("**📝 Description du poste**")
                        desc = row["Description"] if row["Description"] else "Aucune description disponible."
                        st.markdown(desc.replace("\n", "  \n"))
                    with col_b:
                        st.markdown("**📌 Infos clés**")
                        st.markdown(f"- 💼 **Contrat** : {row['Contrat']}")
                        st.markdown(f"- 📍 **Lieu** : {row['Localisation']}")
                        st.markdown(f"- 🎓 **Expérience** : {row['Expérience']}")
                        st.markdown(f"- 💰 **Salaire** : {row['Salaire']}")
                        st.markdown(f"- 🛠️ **Compétences** : {row['Compétences']}")
                        if row["Lien"]:
                            st.link_button("🔗 Voir l'offre originale", row["Lien"])
